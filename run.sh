...
echo "Compile DysfunctionalForm.cs to create the file: DysfunctionalForm.dll"
mcs -target:library -r:System.Windows.Forms.dll -r:System.Drawing.dll -out:DysfunctionalForm.dll DysfunctionalForm.cs

echo "Compile DysfunctionalMain.cs and link the previously created dll file(s) to create an executable (.exe) file."
